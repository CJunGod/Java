...
public class insertEmail extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String msgcontent = request.getParameter("textindex");
		int state = 0;
		String sendto = request.getParameter("sendto");
		Date date = new Date();
		Email email = new Email(username,title,msgcontent,state,sendto,date);
		EmailDao_Server es = new EmailImpl_Server();
		PrintWriter out = response.getWriter();
		if(es.insert(email)>0){
			out.print("<script>");
			...
			out.print("window.location.href='../Enter.jsp'");
			out.print("</script>");
		}
	}
